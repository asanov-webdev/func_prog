...
class Eclat:
    ...
    def transform(self):
        print({k: "{0:.4f}%".format((v[0] + 0.0) / self.data_size * 100) for k, v in self.final_dict.items()})


df = read_file('Online Retail.csv')

df = df[['InvoiceNo', 'Description']]
df['Items'] = df.groupby(['InvoiceNo'])['Description'].transform(lambda x: ','.join(map(str, x)))
df = df[['InvoiceNo', 'Items']]
df.drop_duplicates(subset="Items", keep="first", inplace=True)
df.to_csv('Invoices.csv')

for i, row in df.iterrows():
    row.Items = row.Items.split(',')

df = df[['Items', 'InvoiceNo']]

model = Eclat(min_support = 0.01, max_items = 4, min_items = 2)
